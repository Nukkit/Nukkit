    ...
    public boolean onActivate(Level level, Player player, Block block, Block target, BlockFace face, double fx, double fy, double fz) {
        if (block.getId() == AIR && (target instanceof BlockSolid)) {
            if (target.getId() == Block.OBSIDIAN) {
                ...
                int x_min = targetX;
                int x;
                for (x = targetX + 1; level.getBlock(new Vector3(x, targetY, targetZ)).getId() == Block.OBSIDIAN; x++) {
                    x_max++;
                }
                for (x = targetX - 1; level.getBlock(new Vector3(x, targetY, targetZ)).getId() == Block.OBSIDIAN; x--) {
                    x_min--;
                ...
                int z_min = targetZ;
                int z;
                for (z = targetZ + 1; level.getBlock(new Vector3(targetX, targetY, z)).getId() == Block.OBSIDIAN; z++) {
                    z_max++;
                }
                for (z = targetZ - 1; level.getBlock(new Vector3(targetX, targetY, z)).getId() == Block.OBSIDIAN; z--) {
                    z_min--;
                ...
                int z_min_y = targetY;
                int y;
                for (y = targetY; level.getBlock(new Vector3(targetX, y, z_max)).getId() == Block.OBSIDIAN; y++) {
                    z_max_y++;
                }
                for (y = targetY; level.getBlock(new Vector3(targetX, y, z_min)).getId() == Block.OBSIDIAN; y++) {
                    z_min_y++;
                }
                ...
                if ((count_x >= 4 && count_x <= 23 || count_z >= 4 && count_z <= 23) && count_y >= 5 && count_y <= 23) {
                    int count_up = 0;
                    for (int up_z = z_min; level.getBlock(new Vector3(targetX, y_max, up_z)).getId() == Block.OBSIDIAN && up_z <= z_max; up_z++) {
                        count_up++;
                    }
                    if (count_up == count_z) {
