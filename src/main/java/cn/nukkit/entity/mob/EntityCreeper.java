	...
	public Item[] getDrops() {
		if (this.lastDamageCause instanceof EntityDamageByEntityEvent) {
			...
			if(event.getDamager() instanceof EntityArrow){
				...
				//TODO SKELETON if(arrow.getShootingEntity() instanceof EntitySkeleton)
				//TODO DROP MUSIC DISC
			}

			int dropAmount = this.level.rand.nextInt(2) + 1;
			if (dropAmount == 0)
				return new Item[0];
			return new Item[] { Item.get(Item.GUNPOWDER, dropAmount) };
		}
		return new Item[0];
	}
